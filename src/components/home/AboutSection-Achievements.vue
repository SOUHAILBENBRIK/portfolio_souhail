  ...
  <CollapsiblePanel :header="$t('about.achievements.title')" class="mt-5">
    ...
    <ul class="space-y-10">
      <li v-for="achievement in achievements" :key="achievement.title" class="mx-auto max-w-[80ch]">
        ...
        >
          <time class="source-sans text-sm uppercase text-gray-800 sm:float-end" v-if="achievement.date.includes('/')">
            {{ getDate(achievement.date.split('/')[0])+' - '+getDate(achievement.date.split('/')[1])  }}</time
          >
          <time class="source-sans text-sm uppercase text-gray-800 sm:float-end" v-if="!achievement.date.includes('/')">
